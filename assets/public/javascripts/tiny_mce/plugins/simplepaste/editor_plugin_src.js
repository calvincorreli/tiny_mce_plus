		...
		_cleanPasteClipboard: function(content) {
			...
			if (content && content.length > 0) {
				...
				if (t.editor.getParam('paste_insert_word_content_callback'))
					content = t.editor.execCallback('paste_insert_word_content_callback', 'before', content);
				...
				// LARS 2008-10-03
				var header_tag = t.editor.getParam("paste_convert_headers_to_tag", null);
				if (header_tag) {
  				content = content.replace(new RegExp('<p class=MsoHeading.*?>(.*?)<\/p>', 'gi'), '<'+header_tag+'>$1</'+header_tag+'>');
  			}
